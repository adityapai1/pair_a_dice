  ...
  const PStrong = ({ passwordLength, setPasswordLength, passwordStrength, isGeneratePressed, passphraseGenerated, entropy, setGeneratePressed }) => {
      ...
      <div className="bg-gray-800 m-6 p-4 rounded-lg shadow-md flex flex-col md:flex-row items-center justify-center w-full max-w-2xl">
        ...
        <div className="w-full max-w-md mt-4 md:mt-0">
          ...
          <div className="bg-gray-700 p-4 h-32 rounded-lg shadow-md flex items-center justify-center relative">
            <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-500 ${showEntropy ? 'opacity-100' : 'opacity-0'}`}>
              <span className="text-2xl font-bold text-white">{entropy}</span>
            </div>
